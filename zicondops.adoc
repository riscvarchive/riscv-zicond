...
Otherwise, place the value of register _rs1_ into _rd_.

The instruction behaves as if there were a conditional branch dependent on _rs2_ being equal to zero that resulted in either a move of _rs1_ into _rd_ or a move of the value zero into _rd_, such that the syntactic dependency on _rs1_ is only propagated when the condition is false.

Note that these branch-based semantics do not prevent implementing this instruction as a simple select (e.g., "(rs2==0) ? 0 : rs1").
...
The instruction behaves as if there were a conditional branch dependent on _rs2_ being non-zero that resulted in either a move of _rs1_ into _rd_ or a move of the value zero into _rd_, such that the syntactic dependency on _rs1_ is only propagated when the condition is false.
